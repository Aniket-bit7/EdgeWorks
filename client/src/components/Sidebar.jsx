...
const Avatar = ({ user }) => {
  ...
  return (
    <div className="w-16 h-16 bg-gray-300 rounded-full flex items-center justify-center text-3xl font-bold mx-auto cursor-pointer">
      {Icon ? <Icon className="w-10 h-10 text-gray-700" /> : <span>{letter}</span>}
    </div>
  );
};
...
const SmallAvatar = ({ user }) => {
  ...
  return (
    <div className="w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center text-lg font-bold">
      {Icon ? <Icon className="w-5 h-5 text-gray-700" /> : <span>{letter}</span>}
    </div>
  );
};
  ...
  return (
    ...
    >
      <div className="my-7 w-full text-center relative">
        ...
        {avatarMenu && <AvatarPicker close={() => setAvatarMenu(false)} />}
          ...
          {navItems.map((item) => (
            <div key={item.to} onClick={() => handleNavClick(item)}>
              <NavLink
                ...
                className={({ isActive }) =>
                  `
                    flex items-center justify-between w-full px-3.5 py-2.5 rounded
                    ${isActive ? "bg-black text-white" :
                      item.pro && !isPro ? "text-gray-400 cursor-not-allowed" :
      ...
      <div className="w-full border-t border-gray-200 p-4 px-7 flex items-center justify-between">
        ...
        <LogOut onClick={logout} className="w-4 text-gray-400 hover:text-gray-700 cursor-pointer" />
      </div>
    </div>
  );
};
